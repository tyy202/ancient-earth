...
| 大陆与大洋名称 | 26 个年代的本地中文名称示意，随球体旋转，背面隐藏、文字避让，可随时关闭。 |
| 恐龙分布科普 | 按 PBDB 化石记录展示七个恐龙时代帧的大致发现区域；排除鸟类，本地数据，附来源和取样年代说明。 |
...
- 拖动地球改变观察角度，滚动鼠标滚轮缩放。
- 点击“隐藏地名／显示地名”切换大陆和大洋名称；浅黄色为大陆，浅蓝色为海洋。名称为大致位置示意，部分年代只标主要陆块，详见[数据来源与说明](docs/geography-labels.md)。
- 在“生物”菜单选择“恐龙（非鸟类）”，再切换到 2.2 亿—9000 万年前的现有年代，查看橙色化石发现区域标记。数据按所选年代前后 500 万年汇总，标记是陆块级示意，不是完整栖息范围；详见[恐龙数据说明](docs/dinosaur-distribution.md)。
- 使用顶部下拉菜单或左右方向键切换年代。
